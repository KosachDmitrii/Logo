...
test("ships the finished Loopen product surface", async () => {
  ...
  assert.match(studio, /brief\.generate|Generate 4 logo concepts/);
  assert.match(studio, /studio-gate|auth\.signin\.title\.1|Enter the/);
  assert.match(studio, /signal-pill|signals/);
  assert.match(studio, /strategy\.eyebrow|02 \/ Category research/);
  assert.match(studio, /strategy\.awaiting|Awaiting brief/);
  assert.match(studio, /["'`]\/project-list["'`]/);
  assert.match(studio, /["'`]\/generate-concepts["'`]/);
  assert.match(studio, /confirm\.deleteProject|Delete .* project/);
  assert.match(studio, /concepts\.downloadPng|Download PNG/);
...
test("ships the complete refinement and vector production workflow", async () => {
  ...
  assert.match(studio, /concepts\.reduceSelected|concepts\.reducing|concepts\.reduceLoader/);
  assert.match(studio, /prod\.reconstruct|canReconstruct/);
  assert.doesNotMatch(studio, /Refine selected logos →/);
  assert.match(studio, /prod\.export\.guide|brand-guide|buildBrandGuideHtml/);
  assert.match(studio, /brief\.industry/);
  assert.match(studio, /brief\.companyDoes/);
  assert.match(studio, /brief\.brandName/);
  assert.match(studio, /Brief template|brief\.template|BRIEF_TEMPLATES/);
  assert.match(studio, /BRIEF_TEMPLATES/);
  assert.match(studio, /applyBriefTemplate/);
  assert.match(studio, /New session|nav\.newSession/);
  assert.match(studio, /resetStudioToFresh/);
  assert.match(studio, /clearBriefTemplate/);
  assert.match(studio, /brief\.blankTemplate/);
  assert.match(studio, /notice\.blankBrief|Blank brief/);
  assert.match(studio, /notice\.studioReset|Studio reset/);
  assert.match(studio, /Northline/);
  assert.match(studio, /Voltara/);
  assert.match(studio, /Muchachos/);
  assert.match(studio, /Barber shop/);
  assert.match(studio, /brief\.ph\.brandName|Ketchup, Atelier North/);
  assert.doesNotMatch(studio, /Quick start templates/);
  assert.match(studio, /brief\.coreIdea/);
  assert.match(studio, /brief\.directCompetitors|CompetitorField|suggestCompetitors/);
  ...
  assert.match(studio, /notice\.sessionRestored|Studio session restored/);
  ...
  assert.match(studio, /personality-field|buildBrandGuideHtml/);
  assert.match(studio, /brief\.person\.\$\{key\}\.desc|brief\.person\.\w+\.desc/);
  ...
  assert.match(studio, /prod\.layout\.icon|Icon only/);
  assert.match(studio, /prod\.originalConcept|Original concept/);
  assert.match(studio, /vectorSourceMode/);
  assert.match(studio, /prod\.juryRec|Jury recommendation/);
  assert.match(studio, /prod\.juryUnlocked|SVG is still unlocked/);
  ...
  assert.match(studio, /confirm\.refine\.retry|Retry refinement/);
  assert.doesNotMatch(refine, /assessLogoImage/);
  assert.match(studio, /concepts\.moreCta|More concept/);
  assert.match(studio, /1 selected|selectedConceptIds\.length|concepts\.selectedCount/);
  assert.match(studio, /notice\.selectedForRefine|selected for refinement/);
