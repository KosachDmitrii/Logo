...
test("keeps generation authenticated, persistent, and server-side", async () => {
  ...
  assert.match(route, /form\.append\("safety_tolerance", "5"\)/);
  assert.doesNotMatch(route, /generateWithRetry/);
  assert.match(route, /logo_concept_safety_recovery/);
  assert.match(route, /assessLogoImage/);
  assert.match(route, /reviewStatus/);
  assert.match(route, /requestId/);
  assert.match(route, /even after one neutral recovery attempt/);
